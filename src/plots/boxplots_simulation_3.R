...
data_MSE <- read.table(file=paste(data_dir,"/sim3_MSE_data.txt",sep=""))
data_coeff <- read_rds(file=paste(data_dir,"/sim3_coeff_data.rds",sep=""))
param <- read_rds(file=paste(data_dir,"/sim3_param.rds",sep=""))
...
filename_boxplot_sim3 <- paste(figures_dir,"boxplot_sim3.pdf",sep="")

boxplot_sim3 <- ggplot(data_MSE, aes(x = Method, y = MSE, fill=Method)) +
  geom_boxplot() +
...
ggsave(filename = filename_boxplot_sim3, boxplot_sim3, width = 10, height = 6)
...
data_pcr$Highlight <- ifelse(data_pcr$Index <= p1, "p1",
                             ifelse(data_pcr$Index <= p1 + p2, "p2.1", "p2"))
...
data_pls$Highlight <- ifelse(data_pls$Index <= p1, "p1",
                             ifelse(data_pls$Index <= p1 + p2, "p2.1", "p2"))
...
data_spcr$Highlight <- ifelse(data_spcr$Index <= p1, "p1",
                              ifelse(data_spcr$Index <= p1 + p2, "p2.1", "p2"))
...
combined_plot_coeff <- ggplot(combined_data_coeff,aes(x = Index, y = Coeff_Values, group = dataset, shape = Highlight)) +
  geom_point(size=1) +
...
filename_combined_plot_coeff <- paste(figures_dir,"/combined_plot_coeff_sim3.pdf",sep="")

ggsave(filename = filename_combined_plot_coeff, combined_plot_coeff, width = 10, height = 6)

filename_combined_plot_coeff_filtered <- paste(figures_dir,"/combined_plot_coeff_filtered_sim3.pdf",sep="")

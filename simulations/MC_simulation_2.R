...
sim2_param <-data.frame(t(c(Nsim,N,p,p1,p2,t(X1_shift),t(X2_shift),beta,sigma2_0,sqrt(sigma2_1))),row.names = NULL)

colnames(sim2_param) <- c('Nsims','Nobs','p','p$_1$','p$_2$','$\\mu_1$',
                          '$\\mu_2$','$\\mu_3$','$\\mu_4$','$\\beta$','$\\sigma_1$','$\\sigma_2$')


sim2_tab1 <- xtable(sim2_param,caption = "Simulation Parameters")

filename_sim2_tab1 <- paste(tables_dir,"/sim2_tab1.txt",sep="")

print(sim2_tab1,sanitize.colnames.function = identity, include.rownames=FALSE,file=filename_sim2_tab1)

# Table 2 MSE Results
sim2_summstat_MSE <- data.frame(rbind(compute_summary_stats(pcr_MSE),
                                      compute_summary_stats(pls_MSE),
                                      compute_summary_stats(spcr_MSE),
                                      compute_summary_stats(naive_MSE)))

rownames(sim2_summstat_MSE) <- c("PCR","PLS","SPCR","Naive")

sim2_tab2 <- xtable(sim2_summstat_MSE,caption = "Summary Statistics of Mean Squared Errors")

filename_sim2_tab2 <- paste(tables_dir,"/sim2_tab2.txt",sep="")

print(sim2_tab2,sanitize.colnames.function = identity, include.rownames=FALSE,file=filename_sim2_tab2)

# Table 3 Accuracy of SPCR

sim2_summstat_X <- data.frame(rbind(compute_summary_stats(X_select),
                                    compute_summary_stats(X_p1_select)))

rownames(sim2_summstat_X) <- c("Total","$X_\\{j<p_1\\}$")
colnames(sim2_summstat_X) <- c("Mean","Var","$25^{th}$","$50^{th}$","$75^{th}$")

sim2_tab3 <- xtable(sim2_summstat_X,caption = "Summary Statistics of the Number of Total Selected Variables and 
                    Correctly Selected Variables by the SPC method.")

filename_sim2_tab3 <- paste(tables_dir,"/sim2_tab3.txt",sep="")

print(sim2_tab3,sanitize.colnames.function = identity, sanitize.rownames.function = identity, 
      include.rownames=FALSE,file=filename_sim2_tab3)


### Export MSE and coefficients data for boxplots

filename_sim2_MSE_data <- paste(data_dir,"/sim2_MSE_data.txt",sep="")
filename_sim2_coeff_data <- paste(data_dir,"/sim2_coeff_data.rds",sep="")
filename_sim2_param <- paste(data_dir,"/sim2_param.rds",sep="")

write.table(data.frame(naive_MSE, pcr_MSE, spcr_MSE, pls_MSE),file=filename_sim2_MSE_data)

write_rds(list(coeff_spcr=coeff_spcr,coeff_pcr=coeff_pcr,coeff_pls=coeff_pls),
          file=filename_sim2_coeff_data,
          compress="none")
...
write_rds(sim2_param,file=filename_sim2_param,compress="none")
